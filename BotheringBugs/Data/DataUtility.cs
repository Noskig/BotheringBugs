...
namespace BotheringBugs.Data
{
    public static class DataUtility
    {
        ...
        public static async Task ManageDataAsync(IHost host)
        {
            ...
            var userManagerSvc = svcProvider.GetRequiredService<UserManager<BBUser>>();
        ...
        public static async Task SeedDefaultProjectPriorityAsync(ApplicationDbContext context)
        {
            try
            {
                IList<Models.ProjectPriority> projectPriorities = new List<ProjectPriority>() {
                                                    new ProjectPriority() { Name = BBProjectPriority.Low.ToString() },
                                                    new ProjectPriority() { Name = BBProjectPriority.Medium.ToString() },
                                                    new ProjectPriority() { Name = BBProjectPriority.High.ToString() },
                                                    new ProjectPriority() { Name = BBProjectPriority.Urgent.ToString() },
                };
        ...
        public static async Task SeedDefautProjectsAsync(ApplicationDbContext context)
        {
            ...
            int priorityLow = context.ProjectPriorities.FirstOrDefault(p => p.Name == BBProjectPriority.Low.ToString()).Id;
            int priorityMedium = context.ProjectPriorities.FirstOrDefault(p => p.Name == BBProjectPriority.Medium.ToString()).Id;
            int priorityHigh = context.ProjectPriorities.FirstOrDefault(p => p.Name == BBProjectPriority.High.ToString()).Id;
            int priorityUrgent = context.ProjectPriorities.FirstOrDefault(p => p.Name == BBProjectPriority.Urgent.ToString()).Id;
        ...
        public static async Task SeedDefaultUsersAsync(UserManager<BBUser> userManager)
        {
            //Seed Default Admin User
            var defaultUser = new BBUser
            {
        ...
        public static async Task SeedDemoUsersAsync(UserManager<BBUser> userManager)
        {
        ...
        public static async Task SeedDefaultTicketTypeAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketType> ticketTypes = new List<TicketType>() {
                     new TicketType() { Name = BBTicketType.NewDevelopment.ToString() },      // Ticket involves development of a new, uncoded solution 
                     new TicketType() { Name = BBTicketType.WorkTask.ToString() },            // Ticket involves development of the specific ticket description 
                     new TicketType() { Name = BBTicketType.Defect.ToString()},               // Ticket involves unexpected development/maintenance on a previously designed feature/functionality
                     new TicketType() { Name = BBTicketType.ChangeRequest.ToString() },       // Ticket involves modification development of a previously designed feature/functionality
                     new TicketType() { Name = BBTicketType.Enhancement.ToString() },         // Ticket involves additional development on a previously designed feature or new functionality
                     new TicketType() { Name = BBTicketType.GeneralTask.ToString() }          // Ticket involves no software development but may involve tasks such as configuations, or hardware setup
                };
        ...
        public static async Task SeedDefaultTicketStatusAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketStatus> ticketStatuses = new List<TicketStatus>() {
                    new TicketStatus() { Name = BBTicketStatus.New.ToString() },                 // Newly Created ticket having never been assigned
                    new TicketStatus() { Name = BBTicketStatus.Development.ToString() },         // Ticket is assigned and currently being worked 
                    new TicketStatus() { Name = BBTicketStatus.Testing.ToString()  },            // Ticket is assigned and is currently being tested
                    new TicketStatus() { Name = BBTicketStatus.Resolved.ToString()  },           // Ticket remains assigned to the developer but work in now complete
                };
        ...
        public static async Task SeedDefaultTicketPriorityAsync(ApplicationDbContext context)
        {
            try
            {
                IList<TicketPriority> ticketPriorities = new List<TicketPriority>() {
                                                    new TicketPriority() { Name = BBTicketPriority.Low.ToString()  },
                                                    new TicketPriority() { Name = BBTicketPriority.Medium.ToString() },
                                                    new TicketPriority() { Name = BBTicketPriority.High.ToString()},
                                                    new TicketPriority() { Name = BBTicketPriority.Urgent.ToString()},
                };
        ...
        public static async Task SeedDefautTicketsAsync(ApplicationDbContext context)
        {
            ...
            //Get ticket type Ids
            int typeNewDev = context.TicketTypes.FirstOrDefault(p => p.Name == BBTicketType.NewDevelopment.ToString()).Id;
            int typeWorkTask = context.TicketTypes.FirstOrDefault(p => p.Name == BBTicketType.WorkTask.ToString()).Id;
            int typeDefect = context.TicketTypes.FirstOrDefault(p => p.Name == BBTicketType.Defect.ToString()).Id;
            int typeEnhancement = context.TicketTypes.FirstOrDefault(p => p.Name == BBTicketType.Enhancement.ToString()).Id;
            int typeChangeRequest = context.TicketTypes.FirstOrDefault(p => p.Name == BBTicketType.ChangeRequest.ToString()).Id;

            //Get ticket priority Ids
            int priorityLow = context.TicketPriorities.FirstOrDefault(p => p.Name == BBTicketPriority.Low.ToString()).Id;
            int priorityMedium = context.TicketPriorities.FirstOrDefault(p => p.Name == BBTicketPriority.Medium.ToString()).Id;
            int priorityHigh = context.TicketPriorities.FirstOrDefault(p => p.Name == BBTicketPriority.High.ToString()).Id;
            int priorityUrgent = context.TicketPriorities.FirstOrDefault(p => p.Name == BBTicketPriority.Urgent.ToString()).Id;

            //Get ticket status Ids
            int statusNew = context.TicketStatuses.FirstOrDefault(p => p.Name == BBTicketStatus.New.ToString()).Id;
            int statusDev = context.TicketStatuses.FirstOrDefault(p => p.Name == BBTicketStatus.Development.ToString()).Id;
            int statusTest = context.TicketStatuses.FirstOrDefault(p => p.Name == BBTicketStatus.Testing.ToString()).Id;
            int statusResolved = context.TicketStatuses.FirstOrDefault(p => p.Name == BBTicketStatus.Resolved.ToString()).Id;
